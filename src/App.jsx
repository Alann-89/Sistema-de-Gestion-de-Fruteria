...
const App = () => {
  ...
  const handleSaveSale = (saleData) => {
    const newSale = { id: Date.now(), date: new Date(), folio: 1000 + sales.length + 1, ...saleData };
    setSales([...sales, newSale]);
    setProducts(products.map(p => {
      const soldItem = saleData.items.find(i => i.id === p.id);
      return soldItem ? { ...p, stock: p.stock - soldItem.qty } : p;
    }));
  };

  const handleCancelSale = (saleId) => {
    if (currentUser.role !== 'admin' && currentUser.role !== 'dueño') { alert("Solo administradores pueden cancelar ventas."); return; }
    if (!window.confirm("¿Seguro que desea cancelar esta venta? El stock será devuelto.")) return;
    ...
    <div className="flex h-screen bg-[#FAFAFA] text-[#1A1A1A] font-sans overflow-hidden">
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex-1 overflow-auto p-4 md:p-6 bg-[#FAFAFA]">
          {currentView === 'pos' && <POSModule products={products} users={users} currentUser={currentUser} sales={sales} heldSales={heldSales} setHeldSales={setHeldSales} onSaveSale={handleSaveSale} onCancelSale={handleCancelSale} />}
